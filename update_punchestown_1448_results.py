...
# Race results for 14:48 Punchestown
results = {
    'James The Brave': 1,
    'Lough Nigara': 2,
    'Mojoe': 3,
...
punchestown_1448 = [item for item in items if 'Punchestown' in item.get('course', '') and '14:48' in item.get('race_time', '')]

print(f"\n{'='*70}")
print(f"14:48 PUNCHESTOWN RESULTS UPDATE")
print(f"{'='*70}\n")

updated_count = 0
for horse_item in punchestown_1448:
...
my_tip = sorted(punchestown_1448, key=lambda x: float(x.get('combined_confidence', 0)), reverse=True)[0]
...
if tip_name in results:
    ...
    if tip_position == 1:
        ...
        print(f"   Winner: James The Brave @ 6/4")
        print(f"   Winner was my #2 pick (38/100)")
    else:
        print(f"✗ MY TIP LOST: {tip_name} ({tip_score:.0f}/100) came {tip_position}th")
        print(f"   Winner: James The Brave (my #2 pick, 38/100) @ 6/4")
else:
    print(f"⚠ My tip {tip_name} not in results")

# Overall summary - now 11 races
print(f"\n{'='*70}")
print(f"TODAY'S PERFORMANCE SUMMARY (11 races completed)")
print(f"{'='*70}\n")
