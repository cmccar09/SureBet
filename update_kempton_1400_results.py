...
# Race results for 14:00 Kempton
results = {
    'Popmaster': 1,
    'The Lost King': 2,
    'Nikovo': 3,
...
kempton_1400 = [item for item in items if 'Kempton' in item.get('course', '') and '14:00' in item.get('race_time', '')]

print(f"\n{'='*70}")
print(f"14:00 KEMPTON RESULTS UPDATE")
print(f"{'='*70}\n")

updated_count = 0
for horse_item in kempton_1400:
...
my_tip = sorted(kempton_1400, key=lambda x: float(x.get('combined_confidence', 0)), reverse=True)[0]
...
if tip_name in results:
    ...
    print(f"   Winner: Popmaster @ 15/2")
    print(f"\n   ⚠️ CRITICAL: My highest scorer (76/100 EXCELLENT tier) LOST!")
    print(f"   This breaks today's 60+ pattern (previously 2/2 wins)")

# Overall summary - now 8 races
print(f"\n{'='*70}")
print(f"TODAY'S PERFORMANCE SUMMARY (8 races completed)")
print(f"{'='*70}\n")
...
if total_races > 0:
    ...
    print(f"  60+ wins: {high_score_wins}/{high_score_total}", end='')
    if high_score_total > 0:
        high_win_rate = (high_score_wins / high_score_total) * 100
        print(f" ({high_win_rate:.1f}%)")
        
        if high_win_rate < 50:
            print(f"  ⚠️ 60+ win rate dropped below 50% - pattern broken")
    else:
        print()
    ...
    if 30 <= win_rate <= 40 and 55 <= avg_score <= 70:
        ...
        print(f"⚠ Below target - {win_rate:.1f}% win rate")
