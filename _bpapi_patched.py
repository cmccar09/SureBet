    ...
    try:
        ...
        try:
            subscribers_table.update_item(
                Key={'email': email},
                UpdateExpression='SET last_login = :t ADD login_count :one',
                ExpressionAttributeValues={':t': now_iso, ':one': Decimal('1')},
            )
        except Exception as track_err:
            print(f'registration login tracking error: {track_err}')

        return {'statusCode': 200, 'headers': headers, 'body': json.dumps({
            'success': True,
            'message': 'Registration successful. Welcome to BetBudAI!',
            'user': {'email': email, 'username': username, 'full_name': full_name, 'role': 'free'},
        })}
    except Exception as e:
...
def login_subscriber(headers, event):
    ...
    try:
        ...
        # Track last login time and login count
        now_iso = datetime.utcnow().isoformat() + 'Z'
        try:
            subscribers_table.update_item(
            ...
            print(f'login tracking error: {track_err}')  # non-critical
...
def admin_get_subscribers(headers, event):
    ...
    try:
        ...
        while True:
            ...
            if not last_key:
                break
        # Filter out session tokens and username shadow rows
        items = [
        ...
        # Sort by last_login desc, then joined_at desc
