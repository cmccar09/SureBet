...
def fetch_betfair_odds():
    ...
    if not all([creds['username'], creds['password'], creds['app_key']]):
        print("WARNING: Betfair credentials not configured, using mock data")
    ...
    if not creds['session_token']:
        ...
        if session_token:
            ...
            print("Failed to get session token, using mock data")
            return get_mock_data()
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Betfair API error: {response.status_code}")
            return get_mock_data()
...
def refresh_betfair_session(creds):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if token and result.get('status') == 'SUCCESS':
                print("Session token refreshed successfully")
                return token
        
        print(f"Failed to refresh session token: {response.status_code}")
        return None
...
def get_mock_data():
    ...
    courses = ["Cheltenham", "Ascot", "Newmarket", "Kempton Park", "Leopardstown"]
    prefixes = ["Royal", "Golden", "Thunder", "Speed", "Lucky", "Silver", "Dark"]
    suffixes = ["Flash", "Arrow", "Bolt", "Star", "King", "Spirit", "Dream"]
    ...
    for i in range(3):
        ...
        for j in range(6):
            runners.append({
                ...
                "selection_id": str(100000 + i*100 + j),
                "odds": round(random.uniform(2.5, 12.0), 1)
            })
        
        races.append({
            "market_id": f"1.mock{i}",
            "market_name": "Mock Race",
            "venue": random.choice(courses),
            "race_time": (now + timedelta(minutes=45+i*30)).isoformat() + "Z",
            "runners": runners
        })
