...
# Race results for 13:48 Ludlow
results = {
    'Diamond Du Berlais': 1,
    'Afancy Getaway': 2,
    'Catwalk Girl': 3,
    'Jurassic Pet': 4,
}
...
print(f"\n{'='*70}")
print(f"13:48 LUDLOW RESULTS UPDATE")
print(f"{'='*70}\n")

updated_count = 0
for horse_item in ludlow_1348:
...
my_tip = sorted(ludlow_1348, key=lambda x: float(x.get('combined_confidence', 0)), reverse=True)[0]
...
if tip_name in results:
    ...
    if tip_position == 1:
        ...
        print(f"✓ MY TIP PLACED: {tip_name} ({tip_score:.0f}/100) came {tip_position}rd")
        print(f"   Winner: Diamond Du Berlais @ 30/100")
        print(f"   Note: Winner scored only 44/100 in my analysis")
    else:
        print(f"✗ MY TIP LOST: {tip_name} ({tip_score:.0f}/100) came {tip_position}th")
        print(f"   Winner: Diamond Du Berlais @ 30/100")
else:
    print(f"⚠ My tip {tip_name} not in top 4 results")
    print(f"   Winner: Diamond Du Berlais @ 30/100")

# Overall summary - now 7 races
print(f"\n{'='*70}")
print(f"TODAY'S PERFORMANCE SUMMARY (7 races completed)")
print(f"{'='*70}\n")
