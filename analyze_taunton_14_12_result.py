"""
Analyze Taunton 14:12 result - Feb 3, 2026
"""
import boto3
...
print("="*80)
print("TAUNTON 14:12 RESULT ANALYSIS")
print("="*80)
...
race_horses = [item for item in items if '14:12' in item.get('race_time', '') or 'T14:12' in item.get('race_time', '')]

print(f"\nFound {len(race_horses)} horses in database for this race")

# Actual result
result = {
    '1st': ('Grand Vendetta', '4/5'),
    '2nd': ('Wandering Ego', '13/8'),
    '3rd': ('Joyau Allen', '13/2'),
    '4th': ('Dragons And Demons', '125/1'),
    '5th': ('Brisky Dancer', '18/1')
}
...
if race_horses:
    ...
    print(f"\n  Total horses analyzed: {len(sorted_horses)}/7 ({len(sorted_horses)/7*100:.0f}%)")
    ...
    winner_conf = 0
    for item in sorted_horses:
        if 'Grand Vendetta' in item.get('horse', ''):
    ...
    print("\nKEY FINDINGS:")
    if winner_analyzed:
        print(f"  [+] Winner (Grand Vendetta) WAS analyzed: {winner_conf}/100")
        if winner_conf > 0:
            print(f"      System gave it a score - good!")
        else:
            print(f"      System gave it 0/100 - missed it!")
    else:
        print(f"  [-] Winner (Grand Vendetta) NOT analyzed")
    
    # Check validation
    analyzed_pct = len(sorted_horses) / 7 * 100
    if analyzed_pct < 75:
