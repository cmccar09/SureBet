...
def test_factor(races, factor_name, factor_getter, thresholds=[50, 60, 70, 80, 85, 90]):
    ...
    for threshold in thresholds:
        ...
        if roi > 5:
            print(f"  SUCCESSSUCCESSSUCCESS PROFITABLE!")
        elif roi > 0:
            print(f"  SUCCESS Slightly profitable")
        else:
            print(f"  LOSS Loss-making")
...
for name, min_odds, max_odds in odds_ranges:
    ...
    if roi > 5:
        print(f"  SUCCESSSUCCESSSUCCESS HIGHLY PROFITABLE")
    elif roi > 0:
...
print("SUCCESS Results saved to individual_factor_results.json")
print()
print("="*80)
print("KEY FINDINGS:")
print("="*80)
...
if best_configs and best_configs[0]['roi'] > 10:
    print(f"SUCCESS Found profitable factor: {best_configs[0]['factor']}")
    print(f"  ROI: {best_configs[0]['roi']:.1f}%")
elif best_configs and best_configs[0]['roi'] > 0:
    print(f"WARNING Marginally profitable: {best_configs[0]['factor']}")
    print(f"  ROI: {best_configs[0]['roi']:.1f}%")
else:
