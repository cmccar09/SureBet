...
for result in all_baselines:
    marker = "✓✓✓" if result['roi'] > 10 else "✓✓" if result['roi'] > 5 else "✓" if result['roi'] > 0 else "✗"
...
if current_system_result:
    ...
    if current_rank == 1:
        print("✓✓✓ Current system BEATS all baselines!")
    elif current_system_result['roi'] > 0:
        print(f"⚠️ Current system is profitable but beaten by: {best_baseline['name']}")
        print(f"   Best baseline ROI: {best_baseline['roi']:.1f}%")
    else:
        print(f"❌ Current system is LOSING MONEY")
...
if best_baseline['roi'] < 0:
    print("🚨 CRITICAL: Even the BEST strategy is losing money!")
    print("This suggests:")
    ...
    print(f"✓ At least one strategy is profitable: {best_baseline['name']}")
