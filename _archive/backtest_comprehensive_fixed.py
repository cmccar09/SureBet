...
print("SUCCESS All backtest results loaded")
print()
...
if best_factors:
    ...
    for i, factor in enumerate(best_factors[:3], 1):
        ...
        if factor['roi'] > 10:
            ...
            print("   SUCCESS Marginally profitable")
        else:
            print("   LOSS Loss-making")
else:
    print("ERROR No profitable factors found")
...
if strategies:
    ...
    for strategy in strategies:
        marker = "SUCCESSSUCCESSSUCCESS" if strategy['roi'] > 10 else "SUCCESSSUCCESS" if strategy['roi'] > 5 else "SUCCESS" if strategy['roi'] > 0 else "LOSS"
...
if current_system:
    if current_system['roi'] > 5:
        findings.append("SUCCESS Current system IS profitable (ROI: {:.1f}%)".format(current_system['roi']))
    elif current_system['roi'] > 0:
        findings.append("WARNING Current system marginally profitable (ROI: {:.1f}%)".format(current_system['roi']))
    else:
        findings.append("ERROR Current system LOSING MONEY (ROI: {:.1f}%)".format(current_system['roi']))

# 2. Is it beaten by baselines?
if best_baseline and current_system:
    if best_baseline['roi'] > current_system['roi']:
        findings.append(f"ERROR Current system BEATEN by {best_baseline['name']} ({best_baseline['roi']:.1f}% vs {current_system['roi']:.1f}%)")
    else:
        findings.append(f"SUCCESS Current system BEATS all baselines")

# 3. Are there better factors?
if best_factors:
    best_factor = best_factors[0]
    if current_system and best_factor['roi'] > current_system['roi']:
        findings.append(f"WARNING Single factor ({best_factor['factor']}) outperforms current system ({best_factor['roi']:.1f}% vs {current_system['roi']:.1f}%)")

# 4. Is there any edge at all?
if best_baseline and best_baseline['roi'] < 0:
    findings.append("CRITICAL CRITICAL: Even BEST strategy loses money - NO EDGE DETECTED")
else:
    findings.append(f"SUCCESS Edge exists: Best strategy achieves {best_baseline['roi']:.1f}% ROI")

# 5. Data sufficiency
if len(dataset) < 200:
    findings.append(f"WARNING LIMITED DATA: Only {len(dataset)} races (recommended 500+)")
elif len(dataset) < 500:
    findings.append(f"WARNING MODERATE DATA: {len(dataset)} races (recommended 500+)")
else:
    findings.append(f"SUCCESS SUFFICIENT DATA: {len(dataset)} races")
...
if recommendations:
    ...
    print("SUCCESS System performing acceptably")
...
print()
print("SUCCESS Comprehensive report saved to: comprehensive_backtest_report.json")
