...
# Race results for 14:13 Punchestown
results = {
    'Agameoftwohalves': 1,
    'Good Option': 2,
    'Can Happen': 3,
...
punchestown_1413 = [item for item in items if 'Punchestown' in item.get('course', '') and '14:13' in item.get('race_time', '')]

print(f"\n{'='*70}")
print(f"14:13 PUNCHESTOWN RESULTS UPDATE")
print(f"{'='*70}\n")

updated_count = 0
for horse_item in punchestown_1413:
...
my_tip = sorted(punchestown_1413, key=lambda x: float(x.get('combined_confidence', 0)), reverse=True)[0]
...
if tip_name in results:
    ...
    if tip_position == 1:
        ...
        print(f"   Winner: Agameoftwohalves @ 15/8")
    else:
...
# Overall summary - now 9 races
print(f"\n{'='*70}")
print(f"TODAY'S PERFORMANCE SUMMARY (9 races completed)")
print(f"{'='*70}\n")
